...
if __name__ == "__main__":
    test = 0
    while True:
        print 'Test pass %d' % test
        test += 1

        print '  Testing basic CVP'
        hi = 2**128
        lo = 2**120

        n = 20
        m = 10
        L = matrix(nrows=m, ncols=n)
        for i in range(m):
            for j in range(n):
                L[i,j] = randrange(hi)

        cs = vector([randrange(hi) for _ in range(m)])
        x = cs * L
        y = vector([c + randint(-lo, lo) for c in x])
        assert x == cvp(L, y)

        print '  Testing homogenous LGS over integers'
        hi = 2**128
        lo = 2**96

        n = 40 # num vars
        m = 20 # num equations
        xs = [randrange(lo) for _ in range(n-1)]
        xs.append(1)
        xs = vector(xs)
        ...
        print '  Testing truncated homogenous LGS modulo composite'
        hi = 2**128
        lo = 2**100
        p = hi+1
        ...
        n = 40 # num vars
        m = 35 # num equations
        xs = vector([randrange(hi) for _ in range(n)])
        ys = vector([x - x%lo for x in xs])
        ...
        for i in range(m):
            ...
            A.append(cs)
        A = matrix(ZZ, A)
        assert xs == truncated_lgs(A, ys, p)
        assert xs == ys + small_lgs2(A, (-A*ys)%p, p)

        print '  Testing heterogenous LGS over integers'
        hi = 2**128
        lo = 2**100
        ...
        A = matrix([[randrange(hi) for _ in range(n)] for _ in range(m)])
        c = A*x
        assert x == small_lgs(A, c)

        print '  Testing heterogenous LGS mod p'
        hi = 2**128
        lo = 2**100
        p = next_prime(hi)
        Zp = Integers(p)
        ...
        x = vector([randrange(lo) for _ in range(n)])
        A = matrix([[randrange(p) for _ in range(n)] for _ in range(m)])
        c = vector(ZZ,matrix(Zp,A)*x)
        assert x == small_lgs(A, c, p)
